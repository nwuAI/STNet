...
from .models import EdgeModel, StructureModel, InpaintingModel
...
class EdgeConnect():
    def __init__(self, config):
        ...
        self.structure_model = StructureModel(config).to(config.DEVICE)
        ...
        if self.config.MODE == 2:
            self.test_dataset = Dataset(config, config.TEST_FLIST, config.TEST_STRUCTURE_FLIST, config.TEST_EDGE_FLIST, config.TEST_MASK_FLIST,
                                        augment=False, training=False)
        else:
            self.train_dataset = Dataset(config, config.TRAIN_FLIST, config.TRAIN_STRUCTURE_FLIST, config.TRAIN_EDGE_FLIST, config.TRAIN_MASK_FLIST,
                                      augment=True, training=True)
            self.val_dataset = Dataset(config, config.VAL_FLIST, config.VAL_STRUCTURE_FLIST, config.VAL_EDGE_FLIST, config.VAL_MASK_FLIST,
        ...
        while (keep_training):
            ...
            for items in train_loader:
                ...
                # inpaint model
                    ...
                    # train
                    outputs, gen_loss, dis_loss, logs = self.inpaint_model.process(images, structure, images_gray, edges, masks)
                ...
                # structure model
                    ...
                    # train
                    outputs, gen_loss, dis_loss, logs = self.structure_model.process(images, structure, masks)
                    outputs_merged = (outputs * masks) + (structure * (1 - masks))

                    # metrics
                    psnr = self.psnr(self.postprocess(structure), self.postprocess(outputs_merged))
                    mae = (torch.sum(torch.abs(structure - outputs_merged)) / torch.sum(structure)).float()
                    logs.append(('psnr', psnr.item()))
                    logs.append(('mae', mae.item()))

                    # backward
                    self.structure_model.backward(gen_loss, dis_loss)
                    iteration = self.structure_model.iteration
                ...
                # inpaint with edge model
                    ...
                    if not skip_phase2 or np.random.binomial(1, 0.5) > 0:
                        ...
                        outputs2 = self.structure_model(images, structure, masks)
                        outputs2 = outputs2 * masks + structure * (1 - masks)
                    else:
                        outputs = edges
                        outputs2 = structure
                        gray = images_gray

                    outputs, gen_loss, dis_loss, logs = self.inpaint_model.process(images, outputs2.detach(), gray.detach(), outputs.detach(), masks)
                    outputs_merged = (outputs * masks) + (images * (1 - masks))
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint with edge model / joint model
                ...
                edges, gray = self.edge_model(images_gray, edges, masks)
                structure = self.structure_model(images, structure, masks).detach()
                outputs = self.inpaint_model(images, structure, gray.detach(), edges.detach(), masks)
                outputs = output_align(images, outputs)
                outputs_merged = (outputs * masks) + (images * (1 - masks))
    ...
    def sample(self, it=None):
        ...
        # inpaint model
            ...
            outputs = self.inpaint_model(images, structure, images_gray, edges, masks)
            outputs_merged = (outputs * masks) + (images * (1 - masks))

        elif model == 5:
            iteration = self.structure_model.iteration
            inputs = (structure * (1 - masks)) + masks
            outputs = self.structure_model(images, structure, masks)
            outputs_merged = (outputs * masks) + (structure * (1 - masks))
        ...
        # inpaint with edge model / joint model
            ...
            gray = (gray * masks + gray * (1 - masks)).detach()
            outputs2 = self.structure_model(images, structure, masks).detach()
            outputs2 = outputs2 * masks + structure * (1 - masks).detach()
            outputs = self.inpaint_model(images, outputs2, gray, edges, masks)
            outputs_merged = (outputs * masks) + (images * (1 - masks))
